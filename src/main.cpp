...
int main(int argc, char** argv) {
  std::cout << "ZeroPlus Trading Framework" << std::endl;
  std::cout << "DISCLAIMER: USE THE SOFTWARE AT YOUR OWN RISK\n" << std::endl;
  std::locale mylocale("");
  // load the parameters
  Parameters params("zeroplus.conf");
  if (!params.demoMode) {
  ...
  if (index < 2) {
    std::cout << "ERROR: ZeroPlus needs at least two Bitcoin exchanges. Please edit the config.json file to add new exchanges\n" << std::endl;
    return -1;
  }
  // create the csv file
  std::string currDateTime = printDateTimeFileName();
  std::string csvFileName = "zeroplus_result_" + currDateTime + ".csv";
  ...
  csvFile.flush();
  // create the log file
  std::string logFileName = "zeroplus_log_" + currDateTime + ".log";
  ...
  logFile << "--------------------------------------------" << std::endl;
  logFile << "|   ZeroPlus Trading Framework Log File    |" << std::endl;
  logFile << "--------------------------------------------\n" << std::endl;
  logFile << "ZeroPlus started on " << printDateTime() << "\n" << std::endl;
  ...
  for (int i = 0; i < numExch; ++i) {
    ...
    if (balance[i].btc > 0.0300) {
      logFile << "ERROR: All BTC accounts must be empty before starting ZeroPlus" << std::endl;
      return -1;
    }
  }
  logFile << std::endl;
